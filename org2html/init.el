...
(add-to-list 'load-path "/root/.emacs.d/lisp/emacs-htmlize")

;; highlight.js support
;; from: https://emacs.stackexchange.com/questions/9832/
...
shamelessly copied from org2blog/wp-replace-pre()"
  (save-excursion (let (pos code lang info params header code-start code-end html-attrs pre-class)
					(with-temp-buffer (insert html)
									  (goto-char (point-min))
									  (save-match-data (while (re-search-forward "<pre\\(.*?\\)>"
																				 nil t 1)
...
(defun org2html--render-subtree ()
  ...
  (let ((org-directory default-directory) html-file tags title post-slug html-text)
	...
	(with-temp-file html-file (insert html-text))
	(message "%s created" html-file)))

(defun org2html-export-into-html-text ()
  (let (html-text b e)
	(save-excursion (org-mark-element)
    ...
    (save-excursion
	  (setq html-text (cond ((version-list-< (version-to-list (org-version))
											 '(8 0 0))
							 (if (fboundp 'org-export-region-as-html)
								 (org-export-region-as-html b e t 'string)))
							(t (if (fboundp 'org-export-as)
								   (org-export-as 'html t nil t)))))) html-text))
...
(defun org2html-export-subtree ()
  ...
  (let ((org-directory default-directory) html-file tags title post-slug html-text)
    ...
    (condition-case nil (outline-up-heading 8)
	  (error
...
(eval-after-load 'ox '(progn (add-to-list 'org-export-filter-src-block-functions
